...
public class GuiMiniumChestGUI extends ElementsDemlehcoreMod.ModElement {
	public static int GUIID = 3;
	public static HashMap guistate = new HashMap();
	public GuiMiniumChestGUI(ElementsDemlehcoreMod instance) {
		super(instance, 194);
	}
	...
	public static class GuiContainerMod extends Container implements Supplier<Map<Integer, Slot>> {
		...
		public ItemStack transferStackInSlot(EntityPlayer playerIn, int index) {
			...
			if (slot != null && slot.getHasStack()) {
				...
				if (index < 91) {
					if (!this.mergeItemStack(itemstack1, 91, this.inventorySlots.size(), true)) {
						return ItemStack.EMPTY;
					}
					slot.onSlotChange(itemstack1, itemstack);
				} else if (!this.mergeItemStack(itemstack1, 0, 91, false)) {
					if (index < 91 + 27) {
						if (!this.mergeItemStack(itemstack1, 91 + 27, this.inventorySlots.size(), true)) {
							return ItemStack.EMPTY;
						}
					} else {
						if (!this.mergeItemStack(itemstack1, 91, 91 + 27, false)) {
							return ItemStack.EMPTY;
						}
					}
					return ItemStack.EMPTY;
	...
	public static class GuiWindow extends GuiContainer {
		...
		private static final ResourceLocation texture = new ResourceLocation("demlehcore:textures/minium_chest_gui.png");
		@Override
